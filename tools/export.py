...
def export_to_openvino(cfg, onnx_model_path, output_dir_path, input_shape=None, input_format='bgr', with_text=False):
    ...
    command_line = f'/opt/intel/openvino/deployment_tools/model-optimizer/mo.py --input_model="{onnx_model_path}" ' \
    ...
    try:
        run(f'/opt/intel/openvino/deployment_tools/model-optimizer/mo.py -h', stdout=DEVNULL, stderr=DEVNULL, shell=True, check=True)
    ...
    if with_text:
        ...
        command_line = f'/opt/intel/openvino/deployment_tools/model-optimizer/mo.py --input_model="{onnx_model_path_tr_encoder}" ' \
                       f'--output_dir="{output_dir_path}"'
        print(command_line)
        run(command_line, shell=True, check=True)

        onnx_model_path_tr_decoder = onnx_model_path.replace('.onnx', '_text_recognition_head_decoder.onnx')
        command_line = f'/opt/intel/openvino/deployment_tools/model-optimizer/mo.py --input_model="{onnx_model_path_tr_decoder}" ' \
                       f'--output_dir="{output_dir_path}"'
        print(command_line)
        run(command_line, shell=True, check=True)
